...
public class TokenService {
	...
	@Value("${stima.jwt.expiration}")
	private String expiration;
	@Value("${stima.jwt.secret}")
	private String secret;

	public String gerarToken(Authentication authenticate) {
		Funcionario logado = (Funcionario) authenticate.getPrincipal();
		Date hoje = new Date();
		Date dataExpiration = new Date(hoje.getTime() + Long.parseLong(expiration));
		return Jwts.builder().setIssuer("API connect").setSubject(logado.getId().toString()).setIssuedAt(hoje)
